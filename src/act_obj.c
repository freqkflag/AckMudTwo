...
/* hack to take care of players used to old system */
    if (  ( !is_name( "from", argument ) )
       && ( is_name( "corpse" , argument ) )  )
    {
      sprintf( container_name, "%s", "corpse" );
      if ( is_name( "all", argument ) )
        sprintf( object_list, "1 %s", "all" );
      else if (  ( is_name( "coins", argument ) )
              || ( is_name( "coin", argument ) )
              || ( is_name( "money", argument ) )  )
    ...
    if (  ( container_name[0] != '\0' )
       && ( container = get_obj_here( ch, container_name ) ) == NULL )
    {
    ...
    if ( container != NULL )
    {
         ...
         || ( !str_cmp( "coinsonly", object_list ) )  )
      {
        ...
        {
            act( "There is nothing at all in the $T.",
              ch, NULL, container_name, TO_CHAR );
        }
        else
        {
      ...
      else
      {
        char * parse;
        parse =  object_list;
        sprintf( bug_buf, "Get %s, object_list: %s, container %s", 
          argument,
          object_list,
          container_name );
        ...
        for ( ; ; )          	    /* 'get obj corpse' */
        {
          ...
          if ( one_object[0] == '\0' )
            break;
          sprintf( bug_buf, "Looking for %s %s", 
            object_number, one_object );
          monitor_chan( bug_buf, MONITOR_DEBUG );
          for ( looper = 0; 
                looper < ( is_number( object_number ) ? atoi( object_number ) : 0 );
                looper++ )
          {
    ...
    else
    {
      ...
      for ( ; ; )          	   
      {
        ...
        for ( looper = 0; looper < atoi( object_number ); looper++ )
        {
          ...
          if ( get_obj_weight( obj ) + get_obj_weight( container_obj )
               > container_obj->value[0] )
          {
