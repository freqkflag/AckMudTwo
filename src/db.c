...
#include "imc-config.h" /* for vsnprintf support -- Alty */
...
void load_objfuns( FILE *fp )
{
  for ( ; ; )
  {
	   ...
	   if ( ( pObj = get_obj_index( fread_number( fp ) ) ) == NULL )
     {
       ...
       char buf[MSL];
       temp = fread_word( fp );
       sprintf( buf, "Error in Load Objfuns:  area %s has Objfun without corresponding object.  Save
this area after booting complete to remove.", strArea );
       log_f( buf );
       free_string( temp );
     }
     else
