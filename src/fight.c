...
/* Player vs player bonus, to handle unbalanced hitroll vs ac */
           ...
           || ( IS_NPC(ch) && IS_SET(ch->skills, MOB_ENHANCED ) ) )
    {
        if (IS_NPC(ch))
            ...
            dam * ch->pcdata->learned[gsn_enhanced_damage] / 150 :
            dam * .4;
    }
    if ( !IS_AWAKE(victim) )
	dam *= 1.5;
...
void dam_message( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
    ...
    else if ( dt == TYPE_HIT )
    {
         ...
         else if ( dam <=  50 ) { col="@@c"; vs = "tickle"; 	vp = "tickles";	}
         else if ( dam <=  125 ) { col="@@a"; vs = "glance";	vp = "glances"; }
         else if ( dam <=  275 ) { col="@@G"; vs = "slice";	vp = "slices"; }
         else if ( dam <=  400 ) { col ="@@r"; vs = "strike";	vp = "strikes"; }
         else if ( dam <=  550 ) { col="@@R"; vs = "whallop";	vp = "whallops";}
         else if ( dam <=  800 ) { col="@@e"; vs = "maul";		vp = "mauls";	}
         else                   { col ="@@2@@W"; vs = "thwack";	vp = "thwacks"; }
    ...
    else
    {
      ...
      if ( sysdata.shownumbers == TRUE )
      {
        if ( dt != TYPE_MARTIAL )
        {
          ...
          if ( *str == '\'' )
           sprintf( buf3, "%s$n %s%s your%s%s $s %s%c@@g @@l(@@d%d@@l)@@N",col,col, vp,col, str+2, attack, punct, dam );
          else
