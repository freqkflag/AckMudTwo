  ...
  /*    if ( ch->stunTimer > 0 )
	...
	if ( ( !IS_NPC(ch) && ch->level < LEVEL_IMMORTAL ) )
	{
        ...
        if ( obj->value[7] > 0 )
        {
          ...
          {
            sprintf( bug_buf, "ERROR in expiring item %s(%s %d): item has a replace vnum set (%d),
              but that is not a valid item.",
...
void auction_update( void )
 {
    ...
    switch ( auction_stage )
    {
	  ...
	  {
	     sprintf( buf, "@@N%s (level:%d, valued at %s) has been offered for auction.  A @@e10%% fee@@N will be charged, the higher of the reserve price or highest bid.",
