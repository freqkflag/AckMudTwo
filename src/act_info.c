...
void do_look( CHAR_DATA *ch, char *argument )
{
  ...
  else 
  {
	sprintf( out, "%s%s%s\n\r", color_string( ch, "rooms" ),
		 ch->in_room->name, color_string( ch, "normal" ) );
...
void do_score( CHAR_DATA *ch, char *argument )
{
    ...
    */               
    ...
    else if ( GET_AC(ch) >=   0  ) safe_strcat( MAX_STRING_LENGTH,  buf, "@@ybadly armored."      );
    else if ( GET_AC(ch) >=   -100 ) safe_strcat( MAX_STRING_LENGTH,  buf, "@@yslightly armored."   );
    else if ( GET_AC(ch) >=   -300 ) safe_strcat( MAX_STRING_LENGTH,  buf, "@@ysomewhat armored."   );
    else if ( GET_AC(ch) >=    -500 ) safe_strcat( MAX_STRING_LENGTH,  buf, "@@yarmored."            );
    else if ( GET_AC(ch) >= - 800 ) safe_strcat( MAX_STRING_LENGTH,  buf, "@@ywell armored."       );
    else if ( GET_AC(ch) >= - 1400 ) safe_strcat( MAX_STRING_LENGTH,  buf, "@@ystrongly armored."   );
    else if ( GET_AC(ch) >= - 2000 ) safe_strcat( MAX_STRING_LENGTH,  buf, "@@yheavily armored."    );
    else if ( GET_AC(ch) >= - 3000 ) safe_strcat( MAX_STRING_LENGTH,  buf, "@@ysuperbly armored."   );
    else if ( GET_AC(ch) >= -4500 ) safe_strcat( MAX_STRING_LENGTH,  buf, "@@ydivinely armored."   );
...
void do_who( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf,    
    "The Mighty Adventurers of " mudnamecolor "\n\r" );
	  safe_strcat( MAX_STRING_LENGTH, buf, "@@b|-----------------------------------------------------------------------------+\n\r" );
	  safe_strcat( MAX_STRING_LENGTH, buf, "| @@mSo An Ki Ne Mo@@b                                                              |\n\r" );
	  safe_strcat( MAX_STRING_LENGTH, buf, "| @@cMa Cl Th Wa Ps @@yRace Clan  @@dA@@bB@@rJ@@eP@@WW    @@aAdventurer	   @@mTitle 	    @@r (flags)  @@b|\n\r" );
	  safe_strcat( MAX_STRING_LENGTH, buf, "|---------------------------------|-------------------------------------------|\n\r" );    
	     ...
	     && ( skill_table[sn].flag1 == ADEPT ) )
	{
	  ...
	  if ( ch->pcdata->learned[sn] > 0 )
	  {
	      ...
	      safe_strcat ( MAX_STRING_LENGTH, buf1, buf );
                ...
                ( adept_class ? ch->adept_level : remort_class ?
                  ch->lvl2[class] : ch->lvl[class] ) )
	  {
...
void do_color( CHAR_DATA *ch, char *argument )
{
   ...
   if ( argument[0] == '\0' )
   {
	 ...
	 sprintf( buf, "@@W%8s: %s%-12s@@N   ",
		  color_table[cnt].name,
		  ansi_table[ch->pcdata->color[cnt]].value,
		  ansi_table[ch->pcdata->color[cnt]].name );
	 send_to_char( buf, ch );
	 if ( ++col % 3 == 0 )
	    send_to_char( "\n\r", ch );
      }
	 ...
	 send_to_char( "\n\r", ch );
      ...
      {
	 sprintf( buf, "%s%-12s@@N  ", 
	    IS_SET(ch->config, CONFIG_COLOR) ? ansi_table[cnt].value : "",
	    ansi_table[cnt].name );
	 send_to_char( buf, ch );
	 if ( ++col % 5 == 0 )
	    send_to_char( "\n\r", ch );
