...
void icec_save_channels(void)
{
  ...
  for (c=saved_channel_list; c; c=c->next)
  {
    ...
    if (current)
    {
      ...
      c->local->name=imc_strdup(current->local->name);
    }
...
DEFINE_DO_FUN(do_isetup)
{
  ...
  if (!strcasecmp(cmd, "rename"))
  {
    ...
    if (!arg1[0])
    {
      send_to_char("Syntax: isetup rename <channel>  <newname>\n\r", ch);
      return;
    }
    ...
    for ( saved = saved_channel_list; saved; saved = saved->next )
      if (  ( saved->local )
         && ( !strcasecmp(saved->local->name, chan ) )   )
        break;
    if ( saved )
    {
      imc_strfree( saved->local->name );
      saved->local->name = imc_strdup( arg1 );
    }
    icec_save_channels();
    return;
  }
...
DEFINE_DO_FUN(do_irefresh)
{
  char buf[MAX_STRING_LENGTH];
  ice_channel *c;

  monitor_chan("ICE refreshing channels", MONITOR_IMC); 
  for (c=saved_channel_list; c; c=c->next)
  {
    sprintf(buf, "refresh %s", c->name);
    icec_command( "ICE", buf );
  }
}
...
void icec_showchannel(ice_channel *c, const char *from, const char *txt, int emote)
{
  ...
  if (!c->local)
    return;
  sprintf( namebuf, "%s@@d@@@@@R%s", imc_nameof( from ), imc_mudof( from ) );
  sprintf(buf, emote ? c->local->format2 : c->local->format1, ( str_cmp( imc_mudof( from ), imc_name )? namebuf : imc_nameof( from ) ), color_itom(txt));
  strcat(buf, "\n\r");
...
bool icec_command_hook(CHAR_DATA *ch, const char *command, const char *argument)
{
  ...
  if (!c  || !c->local )
  {
    ...
    if (  ( imc_hasname( command, ch->pcdata->ice_listen ) ) /* channel is configured locally */
       && ( c ) 
       && ( !c->local ) )      
    {
      char combuf[MSL];
      sprintf( combuf, "refresh %s", c->name );
      monitor_chan( combuf, MONITOR_IMC );
      send_to_char( "Channel not active, refresh message sent.\n\r", ch );
      return TRUE;
    }
    else
      return FALSE;
